...
export default function RecruitmentPipeline() {
  ...
  function moveCandidate(id, direction) {
    setCandidates(cands => cands.map(c => {
      if (c.id !== id) return c;
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-6">Recruitment Pipeline</h1>
      <div className="flex flex-wrap gap-4">
        {stages.map(stage => (
          <div
            key={stage}
            className={`w-full sm:w-1/2 md:w-1/3 lg:w-1/5 flex flex-col h-full border border-gray-200 rounded-lg shadow bg-white ${stageColors[stage]}`}
          >
            <h2 className="text-base font-bold mb-3 text-center py-2 border-b border-gray-100 uppercase tracking-wide text-green-800">{stage}</h2>
            <div className="flex flex-col gap-2 flex-1">
              {candidates.filter(c => c.stage === stage).map(c => {
                ...
                return (
                  ...
                  >
                    <span className="inline-block w-8 h-8 rounded-full bg-green-200 flex items-center justify-center text-green-800 font-bold mb-1 text-base">{c.name[0]}</span>
      ...
      <div className="mt-8 text-gray-500 text-sm text-center">Use the arrows to move candidates between stages.</div>
      {/* Detail Modal */}
      {showDetail && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-white rounded shadow-lg p-8 w-full max-w-xs relative">
            ...
            <div className="flex flex-col items-center mb-4">
              <span className="inline-block w-14 h-14 rounded-full bg-green-200 flex items-center justify-center text-green-800 font-bold text-2xl mb-2">{showDetail.name[0]}</span>
              <h2 className="text-lg font-bold mb-1">{showDetail.name}</h2>
              <div className="text-xs text-gray-500 mb-2">ID: {showDetail.id}</div>
            </div>
            <div className="mb-2"><span className="font-semibold">Stage:</span> <span className="ml-1 px-2 py-1 rounded text-xs font-semibold bg-green-100 text-green-800">{showDetail.stage}</span></div>
            <div className="mb-2"><span className="font-semibold">Mentor:</span> <span className="ml-1">{showDetail.mentor || <span className='text-gray-400'>None</span>}</span></div>
          </div>
        </div>
      )}
    </div>
